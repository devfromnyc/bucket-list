...
import {
  stayCategories,
  stayCategoryLabels,
} from "@/lib/stayCategories";
import type { StayCategory } from "@/lib/schema";
import type { EnrichStayResult } from "@/lib/gemini";

type Step = "input" | "loading" | "preview";

export function AddStayWizard({
  initialQuery = "",
}: {
  initialQuery?: string;
}) {
  ...
  const [categoryHint, setCategoryHint] = useState<StayCategory | "">("");
  const [error, setError] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);
  const [draft, setDraft] = useState<EnrichStayResult | null>(null);
  ...
  async function runEnrich(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/enrich-stay", {
        method: "POST",
      ...
      if (!res.ok) throw new Error(data.error || "Could not research stay");
  ...
  async function saveStay() {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Could not save stay");
      router.push("/stays");
      router.refresh();
    } catch (err) {
      ...
      {step === "input" || step === "loading" ? (
        <form onSubmit={runEnrich} className="space-y-5">
          <div>
            ...
            >
              Hotel, Airbnb, or stay description
            </label>
            <input
              id="query"
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              placeholder='e.g. "hotels near downtown Charlotte" or "Airbnb with kitchen Asheville"'
          ...
          <div>
            <label
              ...
              className="mb-2 block text-sm font-medium text-[var(--ink)]"
            >
              Stay type hint (optional)
            </label>
            <select
              id="category"
              value={categoryHint}
              onChange={(e) =>
                setCategoryHint((e.target.value as StayCategory | "") || "")
              }
              className="w-full rounded-2xl border border-[var(--border)] bg-[var(--surface)] px-4 py-3 text-[var(--ink)] outline-none focus:ring-2 focus:ring-[var(--accent)]"
              disabled={step === "loading"}
            >
      ...
      {step === "preview" && draft ? (
        <div className="space-y-5 animate-[fadeUp_0.4s_ease]">
          <p className="text-sm text-[var(--muted)]">
            Review the AI results, tweak anything that looks off, then save.
            Open Maps/listing later for live prices and availability.
          </p>
          ...
          <div className="overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--surface)] shadow-[var(--shadow)]">
            ...
            <div className="space-y-3 p-5">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setDraft({
                      ...draft,
                      category: e.target.value as StayCategory,
                    })
                  }
                  className="w-full rounded-xl border border-[var(--border)] bg-[var(--paper)] px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-[var(--accent)]"
                >
              ...
              <Field
                ...
                onChange={(v) =>
                  setDraft({ ...draft, neighborhood: v || null })
                }
              />
              <Field
                label="City"
              ...
              />
              <Field
                label="Price band (rough)"
                value={draft.priceBand ?? ""}
                onChange={(v) => setDraft({ ...draft, priceBand: v || null })}
              />
              <Field
                label="Listing / booking URL"
                value={draft.bookingUrl ?? ""}
                onChange={(v) => setDraft({ ...draft, bookingUrl: v || null })}
              />
              <Field
          ...
          <div className="flex flex-wrap gap-3">
            ...
            >
              {saving ? "Saving…" : "Looks good — save stay"}
            </button>
          </div>
        </div>
      ) : null}
