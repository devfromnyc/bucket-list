...
import { eventCategoryLabels } from "@/lib/eventCategories";
...
export function EventCard({ event, onToggle, onFavorite, onDelete }: Props) {
    ...
    >
      <div className="relative aspect-[16/10] overflow-hidden bg-[var(--surface-muted)]">
        ...
        <span className="absolute left-3 top-3 rounded-full bg-[var(--ink)]/80 px-3 py-1 text-xs font-medium tracking-wide text-[var(--paper)] backdrop-blur">
          {eventCategoryLabels[category] ?? "Other"}
        </span>
        <button
          type="button"
          onClick={() => onFavorite(event.id)}
          aria-label={
            event.favorited ? "Remove from favorites" : "Add to favorites"
          }
          aria-pressed={event.favorited}
        ...
        >
          <HeartIcon filled={event.favorited} />
        </button>
        {event.isFree ? (
      ...
      <div className="flex flex-1 flex-col gap-3 p-5">
        <div>
          <h2
            className={`font-[family-name:var(--font-display)] text-xl leading-snug text-[var(--ink)] ${
              event.completed ? "line-through decoration-[var(--muted)]" : ""
            }`}
          >
            {event.title}
          </h2>
        ...
        <div className="mt-auto flex flex-wrap items-center gap-2 pt-2">
          ...
          >
            {event.completed ? "Mark upcoming" : "Mark attended"}
          </button>
          <button
            type="button"
